...
MainWindow::MainWindow() : WindowBase(NULL) {
    ...
    for (auto& dirEntry : fs::directory_iterator(backupFolderPath)) {
        if (dirEntry.is_directory()) {
            ...
            wxMenuItem* entry =
                new wxMenuItem(backupRestoreMenu, wxID_ANY, text, wxEmptyString,
                               wxITEM_NORMAL);
            ...
            backupRestoreMenu->Bind(wxEVT_MENU, &MainWindow::RestoreBackup,
                                    this, entry->GetId(), -1, (wxObject*)data);
        }
    }
...
void MainWindow::OpenExtractedFiles(wxCommandEvent& event) {
    wxDirDialog* dialog =
        new wxDirDialog(this, "Open WII/PS3/X360 folder", "",
...
void MainWindow::ParseExtracted(fs::path path) {
    ...
    if (fs::exists(tracklistingPath) && fs::exists(tracIDPath) &&
        fs::exists(tracEPath)) {
        ...
        wxLogMessage(wxString("Successfully loaded ")
                     << count << " songs from tracklisting.");
        ...
        baseFolderLabel->SetLabelText(wxString("Base Folder:  ")
                                      << path.c_str());
            ...
            while (tracklistingTrack != nullptr) {
                ...
                if (IDTag != nullptr) {
                    ...
                    if (trackTestingID == addingTag) {
                        wxLogMessage(wxString("Replacing track node: ")
                                     << addingTag);
...
void MainWindow::TracksToCustoms(wxCommandEvent& event) {
    wxDirDialog* dialog =
        new wxDirDialog(this, "Select a folder to export the customs to", "",
                        wxDD_DEFAULT_STYLE | wxDD_DIR_MUST_EXIST);
    if (dialog->ShowModal() == wxID_OK) {
...
void MainWindow::SetBackupFolder(wxCommandEvent& event) {
    wxDirDialog* dialog =
        new wxDirDialog(this, "Open Custom's folder", "",
                        wxDD_DEFAULT_STYLE | wxDD_DIR_MUST_EXIST);
    if (dialog->ShowModal() == wxID_OK) {
...
void MainWindow::ManualBackup(wxCommandEvent& event) {
    std::string text =
        wxGetTextFromUser("Please enter the name of the backup:", "Caption",
                          wxEmptyString, this)
            .ToStdString();
    CreateBackup(backupFolderPath, text);
}

void MainWindow::CreateBackup(std::filesystem::path baseFolder,
                              std::string name) {
    if (fs::exists(baseFolder)) {
        ...
        wxMenuItem* entry = new wxMenuItem(backupRestoreMenu, wxID_ANY, name,
                                           wxEmptyString, wxITEM_NORMAL);
        wxStringClientData* data =
            new wxStringClientData(wxString(thisBackup.string()));

        backupRestoreMenu->Append(entry);
        backupRestoreMenu->Bind(wxEVT_MENU, &MainWindow::RestoreBackup, this,
                                entry->GetId(), -1, (wxObject*)data);
    } else {
        wxLogWarning(
    ...
    if (!basePath.empty() && dirty) {
        ...
        if (a.compare(b) == 0) {
            ...
            if (!automaticRenaming) {
                ...
                dialog.SetYesNoCancelLabels("Yes, don't ask me again", "Yes",
                                            "No");
...
void MainWindow::ToggleAutomaticRenaming(wxCommandEvent& event) {
    ...
    mINI::INIStructure ini;
    settings.read(ini);
    ini["Settings"]["automaticRenaming"] =
        (automaticRenaming ? "true" : "false");
    settings.write(ini);
...
void MainWindow::ToUpper(wxCommandEvent& event) {
    if (!basePath.empty()) {
        ...
        if (filesystem.find(L"FAT", 0) != std::wstring::npos) {
            std::wstring message =
                L"Unsupported Filesystem detected: " + filesystem + L"\n";
            message +=
                L"Renaming a file to the same name in uppercase does not work "
                L"on FAT filesystems.\n";
            message +=
                L"Please move the files so they are inside a disk formatted "
                L"differently.\n";
            wxMessageDialog* confirm = new wxMessageDialog(
                this, message, "Invalid file system format found", wxOK);
            if (confirm->ShowModal())
                return;
        }

        wxProgressDialog* dialog = new wxProgressDialog(
            "Progress", "Renaming the files. Please wait a few seconds", 100,
            this);
        dialog->Show();
        dialog->Pulse();
...
void MainWindow::ApplyPatchFile(wxCommandEvent& event) {
    ...
    wxFileDialog dlg(this, "Select Patch File", "", "",
                     "XML Files (*.xml)|*.xml");
                ...
                if (idtag != nullptr) {
                    ...
                    while (query != nullptr) {
                        ...
                        if (strcmp(idtag, queryID) == 0) {
                            // we found the custom
                            wxLogMessage(wxString("Updated Track with id ")
                                         << idtag);
...
MainWindow::~MainWindow() { curl_easy_cleanup(curlSession); }